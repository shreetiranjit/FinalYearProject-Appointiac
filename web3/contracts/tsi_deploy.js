...
//crack process tissue unable secret rubber remember april soon lion screen feed night capital scrub grow sing limit display thought lazy caught chef above remove
...
async function main() {
  ...
  const approvalProgramSource = fs.readFileSync('timeslot_approval_program.teal', 'utf8');
  const compiledApprovalProgram = await compileProgram(algodClient, approvalProgramSource);
  const clearStateProgramSource = fs.readFileSync('timeslot_clear_state_program.teal', 'utf8');
